...
def emd_lp(a, b, M, numItermax=100000, log=False, center_dual=True, numThreads=1, check_marginals=True):
...
def opt_lp(mu,nu,M,Lambda,nb_dummies=1,log=False,**kwargs):
    ...
    mu_extended = np.append(mu[idx_x], [(np.sum(mu) - np.sum(mu[idx_x]) +
                                           np.sum(nu)) / nb_dummies] * nb_dummies)
    nu_extended = np.append(nu[idx_y], [(np.sum(nu) - np.sum(nu[idx_y]) +
                                           np.sum(mu)) / nb_dummies] * nb_dummies)
    
    #print(mu_extended.sum())
    #print(nu_extended.sum())
    
    M_extended=np.zeros((mu_extended.shape[0],nu_extended.shape[0]),dtype=np.float64)
    M_extended[:idx_x.shape[0], :idx_y.shape[0]]= M_star[np.ix_(idx_x, idx_y)]
    gamma_extended,log_dict=emd_lp(mu_extended,nu_extended,M_extended,log=log,**kwargs)
    Gamma=np.zeros((n,m))
